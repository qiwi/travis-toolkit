...
export function active(baseUrl: string, token: string) {
  function forOwnerAction(
    ...
    if (
      ...
      return axios['get'](
        `${baseUrl}/owner/${data.templateVariable['provider']}/${data.templateVariable['login']}/active`,
        {
          headers: {
            'Travis-API-Version': 3,
            Authorization: `${token}`,
    ...
    if (
      ...
      return axios['get'](
        `${baseUrl}/owner/${data.templateVariable['login']}/active`,
        {
          headers: {
            'Travis-API-Version': 3,
            Authorization: `${token}`,
